import electron, { app as app$1, ipcMain as ipcMain$1, BrowserWindow, globalShortcut, protocol } from "electron";
...
function writeFileSync(filePath, data2, options = DEFAULT_WRITE_OPTIONS) {
  if (isString(options))
    return writeFileSync(filePath, data2, { encoding: options });
  ...
  try {
    ...
    if (isString(data2)) {
      FS.retry.writeSync(retryOptions)(fd, data2, 0, options.encoding || DEFAULT_ENCODING);
    } else if (!isUndefined(data2)) {
      FS.retry.writeSync(retryOptions)(fd, data2, 0, data2.length, 0);
    }
    ...
    code(cxt) {
      ...
      function loopUntilMissing(missing, valid2) {
        ...
        gen.forOf(missing, schemaCode, () => {
          gen.assign(valid2, (0, code_1.propertyInData)(gen, data2, missing, opts.ownProperties));
          gen.if((0, codegen_1.not)(valid2), () => {
            cxt.error();
            gen.break();
          });
...
function require_enum$1() {
  ...
  const def = {
    ...
    code(cxt) {
      ...
      function loopEnum() {
        gen.assign(valid2, false);
        gen.forOf("v", schemaCode, (v) => gen.if((0, codegen_1._)`${getEql()}(${data2}, ${v})`, () => gen.assign(valid2, true).break()));
      }
  ...
  (function(exports$1) {
    ...
    function validatePropertyDeps(cxt, propertyDeps = cxt.schema) {
      ...
      for (const prop in propertyDeps) {
        ...
        if (deps.length === 0)
          continue;
        const hasProperty2 = (0, code_1.propertyInData)(gen, data2, prop, it.opts.ownProperties);
        cxt.setParams({
          property: prop,
...
function requireLimit() {
  ...
  (function(exports$1) {
    ...
    exports$1.formatLimitDefinition = {
      ...
      error,
      code(cxt) {
        const { gen, data: data2, schemaCode, keyword: keyword2, it } = cxt;
        const { opts, self } = it;
        if (!opts.validateFormats)
          return;
...
const isExist = (data2) => data2 !== void 0;
...
class Conf {
  ...
  get store() {
    try {
      const data2 = fs.readFileSync(this.path, this.#encryptionKey ? null : "utf8");
      const dataString = this._decryptData(data2);
  ...
  */
  ...
  _decryptData(data2) {
    if (!this.#encryptionKey) {
      return typeof data2 === "string" ? data2 : uint8ArrayToString(data2);
    }
    try {
      const initializationVector = data2.slice(0, 16);
      const password = crypto.pbkdf2Sync(this.#encryptionKey, initializationVector, 1e4, 32, "sha512");
      const decipher = crypto.createDecipheriv(encryptionAlgorithm, password, initializationVector);
      const slice = data2.slice(17);
  ...
  _validate(data2) {
    ...
    const valid2 = this.#validator(data2);
  ...
  _write(value) {
    let data2 = this._serialize(value);
    if (this.#encryptionKey) {
      const initializationVector = crypto.randomBytes(16);
      const password = crypto.pbkdf2Sync(this.#encryptionKey, initializationVector, 1e4, 32, "sha512");
      const cipher = crypto.createCipheriv(encryptionAlgorithm, password, initializationVector);
      data2 = concatUint8Arrays([initializationVector, stringToUint8Array(":"), cipher.update(stringToUint8Array(data2)), cipher.final()]);
    }
    if (process$1.env.SNAP) {
      fs.writeFileSync(this.path, data2, { mode: this.#options.configFileMode });
    } else {
      try {
        writeFileSync(this.path, data2, { mode: this.#options.configFileMode });
      } catch (error) {
        if (error?.code === "EXDEV") {
          fs.writeFileSync(this.path, data2, { mode: this.#options.configFileMode });
          return;
...
let win;
let authWin;
app$1.setName("Locora");
app$1.setAppUserModelId("org.locora.app");
...
ipcMain$1.handle("open-authentication-window", async () => {
...
ipcMain$1.on("window-action", (_, action) => {
  if (!win) return;
  switch (action) {
    case "close":
      win.close();
