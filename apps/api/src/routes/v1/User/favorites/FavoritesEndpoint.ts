...
export default async function FavoritesEndpoint(req: Request, env: Env) {
    ...
    if (segments[0] === "getfavorites") {
        return GetUsersFavorites(req, env)
    } else if (segments[0] === "removefavorite") {
        return RemoveFavorites(req,env)
    } else if (segments[0] === "addfavorite") {
        return await AddFavorite(req, env)
    } else {
        return NotFound(req)
    }
