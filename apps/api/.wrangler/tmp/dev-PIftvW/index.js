...
function JSONResponse(data, status = 200) {
  return Corsify_default(new Response(
    JSON.stringify(data),
...
router2.post("/add", Add_To_Waitlist);
...
async function Add_To_Waitlist(req) {
  const body = await req.json();
  const Email = body.email;
  const IP = req.headers.get("CF-Connecting-IP");
  if (!await checkForValidEmail(Email)) {
    return await checkForValidEmail(Email);
  }
  if (!await VerifyTurnstileToken(body.turnstile_token, IP)) {
    return await VerifyTurnstileToken(body.turnstile_token, IP);
  }
  return Corsify_default(
    JSONResponse({
      status: "success",
...
var handleWaitlist = router2.handle;
...
router3.all("/v1/auth/*", (req) => {
  return handleAuth(req);
});
router3.all("/v1/waitlist/*", (req) => {
  return handleWaitlist(req);
});
router3.all("/v1/*", () => Corsify_default(new Response("Not Found", { status: 404 })));
router3.all("*", () => Corsify_default(new Response("Not Found", { status: 404 })));
var src_default = {
  fetch: /* @__PURE__ */ __name((request, env, ctx) => {
    if (!SourceValidation(request)) {
      return Corsify_default(new Response("Forbidden", { status: 403 }));
    }
...
// .wrangler/tmp/bundle-0CSnjq/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-0CSnjq/middleware-loader.entry.ts
